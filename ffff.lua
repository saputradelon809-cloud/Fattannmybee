-- FATTAN HUB - FINAL ALL IN ONE
-- Password: fattanhubGG
-- GUI Simple + Ringkas/Lengkap + Owner/Member auto title
...
local function createLogin(onSuccess)
    ...
    local function tryLogin()
        if tostring(box.Text) == correctPassword then
            gui:Destroy()
            pcall(onSuccess)
        else
            box.Text = ""
            status.Text = "❌ Password salah!"
            task.delay(1.4,function() if status.Parent then status.Text = "" end end)
        end
    end
    btn.MouseButton1Click:Connect(tryLogin)
    box.FocusLost:Connect(function(e) if e then tryLogin() end end)
end
...
local function initMain()
    -- loading
    local lg = Instance.new("ScreenGui", CoreGui)
    lg.Name = "FattanLoading"
    local f = Instance.new("Frame", lg)
    f.Size = UDim2.new(1,0,1,0)
    f.BackgroundColor3 = Color3.fromRGB(6,36,90)
    ...
    TweenService:Create(l,TweenInfo.new(0.6),{TextTransparency=1}):Play()
    ...
    frame.BackgroundColor3 = Color3.fromRGB(10,40,100)
    frame.Active = true
    frame.Draggable = true

    local title = Instance.new("TextLabel", frame)
    title.Size = UDim2.new(1,0,0,36)
    title.BackgroundColor3 = Color3.fromRGB(20,80,160)
    title.Font = Enum.Font.GothamBold
    title.TextSize = 20
    title.TextColor3 = Color3.new(1,1,1)
    ...
    upBtn.Size = UDim2.new(0.5,0,0,28)
    upBtn.Position = UDim2.new(0,0,1,-28)
    upBtn.Text = "⬆ Ringkas"
    upBtn.Font = Enum.Font.GothamBold
    upBtn.TextSize = 16
    upBtn.BackgroundColor3 = Color3.fromRGB(60,120,200)
    upBtn.TextColor3 = Color3.new(1,1,1)

    local downBtn = Instance.new("TextButton", frame)
    downBtn.Size = UDim2.new(0.5,0,0,28)
    downBtn.Position = UDim2.new(0.5,0,1,-28)
    downBtn.Text = "⬇ Lengkap"
    downBtn.Font = Enum.Font.GothamBold
    downBtn.TextSize = 16
    downBtn.BackgroundColor3 = Color3.fromRGB(80,180,80)
    ...
    createButton("Freeze Selected",function()
        ...
        if hum then hum.WalkSpeed=0 hum.JumpPower=0 task.delay(5,function() hum.WalkSpeed=16 hum.JumpPower=50 end) end
    end)
    ...
    createButton("Stop Rope",function()
        if selected and rope[selected] then
            for _,v in pairs(rope[selected]) do if typeof(v)=="RBXScriptConnection" then v:Disconnect() elseif typeof(v)=="Instance" and v.Parent then v:Destroy() end end
            rope[selected]=nil
        end
    end)
    ...
    ----------------------------------------------------
    -- Tombol Ringkas/Lengkap
    ----------------------------------------------------
    local simple=true
    local function setMode(ringkas)
        simple=ringkas
        for _,b in pairs(list:GetChildren()) do
            if b:IsA("TextButton") then b.Visible=not ringkas end
        end
    end
    setMode(true)
