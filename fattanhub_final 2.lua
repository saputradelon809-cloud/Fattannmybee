...
local function initMain()
    ...
    -- ============================
    -- EXTRA FEATURES (ditambahkan tanpa mengubah fitur lama)
    -- ============================
    ...
    local extraLabel = Instance.new("TextLabel", extraFrame)
    extraLabel.Size = UDim2.new(1,0,0,24)
    extraLabel.Position = UDim2.new(0,0,0,4)
    ...
    local efLayout = Instance.new("UIListLayout", extraFrame)
    efLayout.Padding = UDim.new(0,6)
    efLayout.SortOrder = Enum.SortOrder.LayoutOrder
    efLayout.VerticalAlignment = Enum.VerticalAlignment.Top
    efLayout.HorizontalAlignment = Enum.HorizontalAlignment.Center

    -- God Mode
    local godToggle = false
    local godBtn = Instance.new("TextButton", extraFrame)
    godBtn.Size = UDim2.new(0.9,0,0,28)
    godBtn.Position = UDim2.new(0.05,0,0,34)
    ...
    godBtn.MouseButton1Click:Connect(function()
        ...
        if godToggle then
            -- maintain health loop
            task.spawn(function()
                while godToggle do
                    local char = safeChar()
                    if char and char:FindFirstChildOfClass("Humanoid") then
                        pcall(function() char:FindFirstChildOfClass("Humanoid").Health = math.huge end)
                    end
                    task.wait(0.5)
                end
            end)
    ...
    local arBtn = Instance.new("TextButton", extraFrame)
    arBtn.Size = UDim2.new(0.9,0,0,28)
    arBtn.Position = UDim2.new(0.05,0,0,68)
    arBtn.Text = "Auto Respawn: ON"
    arBtn.Font = Enum.Font.SourceSansBold
    arBtn.TextSize = 14
    arBtn.BackgroundColor3 = Color3.fromRGB(80,80,80)
    arBtn.TextColor3 = Color3.new(1,1,1)
    arBtn.AutoButtonColor = true

    arBtn.MouseButton1Click:Connect(function()
        autoRespawn = not autoRespawn
        arBtn.Text = autoRespawn and "Auto Respawn: ON" or "Auto Respawn: OFF"
    end)

    -- Implement respawn behavior: use Player:LoadCharacter when character dies
    ...
    -- Bind to Died event to respawn immediately if toggle enabled
    local function monitorHumanoid(hum)
    ...
    -- attach to current and future humanoid
    ...
    -- Auto Rejoin (manual rejoin button + best-effort auto attempt on disconnect)
    local rejoinBtn = Instance.new("TextButton", extraFrame)
    rejoinBtn.Size = UDim2.new(0.9,0,0,28)
    rejoinBtn.Position = UDim2.new(0.05,0,0,102)
    ...
    -- Best-effort: attempt to rejoin if LocalPlayer is removed from Players (may not fire in all cases)
    -- Note: automatic rejoin after being kicked is not reliably possible from a LocalScript in all situations.
    local autoRejoinFlag = false
    -- you can toggle auto rejoin by shift+R for example (not visible button)
    UserInputService.InputBegan:Connect(function(input, processed)
        if processed then return end
        if input.KeyCode == Enum.KeyCode.R and UserInputService:IsKeyDown(Enum.KeyCode.LeftShift) then
            autoRejoinFlag = not autoRejoinFlag
            -- feedback in output
            print("AutoRejoin flag:", autoRejoinFlag)
        end
    end)
    -- Monitor Players list removal as a heuristic: (best-effort)
    Players.PlayerRemoving:Connect(function(pl)
        if pl == LocalPlayer and autoRejoinFlag then
            pcall(function()
                local tp = game:GetService("TeleportService")
    ...
    antiBtn.TextSize = 12
    antiBtn.BackgroundColor3 = Color3.fromRGB(80,80,80)
