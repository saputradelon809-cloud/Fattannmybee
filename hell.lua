-- ✅ IZZHUB👑 AutoExec Version (AutoRespawn Loop)
-- Simpan file ini di folder autoexec executor kamu

local Players = game:GetService("Players")
local player = Players.LocalPlayer

-- 📍 Koordinat (custom list)
local checkpoints = {
    Vector3.new(-90, 201, 269),
    Vector3.new(154, 249, 424),
    Vector3.new(480, 353, 307),
    Vector3.new(526, 413, -315),
    Vector3.new(-226, 541, -756),
    Vector3.new(-654, 541, -652),
    Vector3.new(-674, 429, -6),
    Vector3.new(-817, 434, 439),
    Vector3.new(-321, 409, 659),
    Vector3.new(345, 329, 716),
    Vector3.new(807, 637, 864),
    Vector3.new(1253, 825, 394),
    Vector3.new(1509, 1193, 18),
    Vector3.new(1000, 1193, -59),
    Vector3.new(-1509, 1873, -72)
}
...
local function autoTP(statusLabel)
    while running do
        for i, pos in ipairs(checkpoints) do
            ...
            -- kalau sudah sampai CP terakhir
            if i == #checkpoints and running then
                statusLabel.Text = "🏔️ Summit Tercapai!"
                task.wait(waitAfterSummit)
                -- respawn ulang player
